    ...
    transformSchema(schema) {
        return (0, utils_1.mapSchema)(schema, {
            ...
            ...(this.config.enumValues && {
                [utils_1.MapperKind.ARGUMENT]: config => {
                    const shouldRenameEnumDefaultValue = this.config.enumValues &&
                        config.astNode?.defaultValue?.kind === graphql_1.Kind.ENUM &&
                        typeof config.defaultValue === 'string' &&
                        config.defaultValue;
                    if (shouldRenameEnumDefaultValue) {
                        const applyNamingConvention = shared_js_1.NAMING_CONVENTIONS[this.config.enumValues];

...
import { Response, fetch, Request } from '@whatwg-node/fetch';
...
const getReqResMapByContext = memoize1((context) => {
    return new Map();
});
...
function useDeduplicateRequestsFetch(nonDuplicateFetch) {
    return function fetchWithDeduplicatedRequests(url, options = {}, context, info) {
        if (context == null) {
...
function useHttpDetailsFetch(nonDetailedFetch) {
    return function fetchWithHttpDetails(url, options = {}, context, info) {
        if (context == null) {
...
const createDefaultMeshFetch = memoize1(function createDefaultMeshFetch(cache) {
    return useHttpDetailsFetch(useDeduplicateRequestsFetch(fetchFactory({
        cache,
        fetch,
        Request,
        Response,
...
export { DefaultLogger, PubSub, applyRequestTransforms, applyResultTransforms, applySchemaTransforms, createDefaultMeshFetch, createLruCache, debugColor, defaultImportFn, errorColor, extractResolvers, fileURLToPath, getHeadersObj, gql, groupTransforms, httpDetailsByContext, infoColor, isUrl, loadFromModuleExportExpression, loadYaml, mkdir, parseWithCache, pathExists, printWithCache, pushHttpDetails, readFile, readFileOrUrl, readUrl, resolveAdditionalResolvers, resolveAdditionalResolversWithoutImport, rmdirs, sanitizeNameForGraphQL, titleBold, useDeduplicateRequestsFetch, useHttpDetailsFetch, warnColor, withCancel, writeFile, writeJSON };
